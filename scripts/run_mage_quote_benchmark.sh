...
SAMPLE_DIR="$ROOT/SampleCode/MutualAttestationQuoteBenchmark"
RESULTS_DIR="${RESULTS_DIR:-$ROOT/results/quote_benchmark}"
...
export WARMUP_QUOTE
echo "[config] results=$RESULTS_DIR collateral_cache=$COLLATERAL_CACHE_DIR parties=\"$PARTIES_LIST\" rounds=$ROUNDS rebuild=$REBUILD warmup_quote=$WARMUP_QUOTE"
...
python3 "$ROOT/scripts/collect_mage_quote_benchmark.py" "$RESULTS_DIR"
