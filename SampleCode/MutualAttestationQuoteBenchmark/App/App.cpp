...
bool parse_options(int argc, char *argv[], options_t *options)
{
    ...
    int opt = 0;
    opterr = 0;
    while ((opt = getopt_long(argc, argv, "i:p:q:r:t:f:c:h:", kLongOptions, NULL)) != -1) {
        switch (opt) {
        case 'i':
...
{
    fprintf(stderr,
            "usage: %s --party-id N --parties N --quote-dir DIR --result-dir DIR --fmspc HEX12 [--pck-ca platform|processor] [--timeout-sec SEC] [--helper PATH]\n",
            program);
}
...
int run_party(const options_t &options)
{
    ...
    }
    {
        ...
        if (!wait_for_all_quotes(options)) {
            timings.wait_all_quotes_ms = elapsed_ms(wait_start_ns, monotonic_now_ns());
            error_text = "wait_for_all_quotes_timeout";
            goto done;
        }
        timings.wait_all_quotes_ms = elapsed_ms(wait_start_ns, monotonic_now_ns());
    }
